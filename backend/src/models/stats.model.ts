...
const getSensorStats = async (deviceId: number, userId: string, range?: string) => {
  ...
  if (range) {
    ...
    if (isNaN(days) || days < 1) return { error: 'Invalid range format, use: 7d, 30d ..' };
  }
  ...
  if (readings.length === 0) return { max: null, min: null, average: null };
...
const getDevicesStatusCount = (userId: string) => {
